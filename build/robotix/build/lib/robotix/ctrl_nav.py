...
class Ctrl_nav(Node):
    ...
    def my_callback_real_pos(self, arrived:Int16):
        if self.index == 2:
            ...
            self.plante_format((self.distance_plante_1[i1])*10+90, self.angle_plante_1[i1]) # plante_format publie sur cmd_pos donc j'ai fait plante_format reverse
            self.index +=1
           # rclpy.sleep(5)
        elif self.index == 3: # plante format reverse du coup ? j'ai juste fait la fonction je ne l'ai pas appelée 
            ...
            self.index += 1 # je pense pas que il y a besoin d'incrémenter (pour moi c'est quand index = 6 qu'on retourne dans le tableau) 
        else :
            self.my_publish()
    ...
    def plante_format_reverse(self, distance, angle):
        #msg = String()
        if angle >= 0:
            str_var1 = f"{int(distance):04d}"  # Formatage sur 4 caractères      
            str_var2 = f"{int(angle-5):03d}"  # Formatage sur 3 caractères       
            self.msg.data = f"R{str_var1}P{str_var2}"

        else:
            str_var1 = f"{int(distance):04d}"  # Formatage sur 4 caractères      
            str_var2 = f"{int(abs(angle+3)):03d}"  # Formatage sur 3 caractères  
            self.msg.data = f"R{str_var1}N{str_var2}"
        print(self.msg.data)
        self.publisher_cmd_pos.publish(self.msg)
